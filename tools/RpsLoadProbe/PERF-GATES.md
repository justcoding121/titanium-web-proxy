...
- **Product gates (current):** [`validate-compare-product-gates.ps1`](validate-compare-product-gates.ps1) — **MITM Lite ÷ Reverse ≥ 0.65**, **Full ÷ Reverse ≥ 0.55**, and **reverse TWP ÷ YARP ≥ 0.95** on **all OS** (no Mac carve-outs; no nginx gate). Spot (`run-spot-matrix.ps1`) checks Full ÷ Reverse ≥ 0.55 and the same YARP floor. Linux H3→H3 YARP peer SLO-fail is skipped (harness). No cross-OS absolute-RPS gates. Fill `wiki/Performance.md` via `paste-compare-product-wiki.ps1` / `apply-wiki-paste.ps1`.
