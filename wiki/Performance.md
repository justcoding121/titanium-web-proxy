...
Median of **3 repeats** on `windows-latest` (4 vCPU / 16 GiB). Saturation @ `f9769503` — [32737672381](https://github.com/justcoding121/titanium-web-proxy/actions/runs/32737672381); bridges [32737668291](https://github.com/justcoding121/titanium-web-proxy/actions/runs/32737668291). **H3→H1** remasure @ `74ef56ee` — [32750437463](https://github.com/justcoding121/titanium-web-proxy/actions/runs/32750437463) (`compare-http3-cleartext`, same-job TWP+YARP). Three-process harness, parent-seeded loopback CA. Warmup 2s / measure 8s; concurrency 8, 16, 32, 64. Prefer TWP÷peer ratios over absolute RPS. **RPS cells** include median RSS / CPU at the peak-RPS step as `<br><sub>(MiB / CPU%)</sub>` for TWP, nginx, and YARP (proxy child + descendant tree). Other bridge / H1 plain / terminate / MITM rows below are still the prior paste. Laptop High-perf / cool-paired numbers stay on the [local lab](Performance-Local-Lab).
...
| Reverse | HTTP/3 · QUIC | HTTP/1 · plain | **15,968**<br><sub>(102 MiB / 47.9% CPU)</sub> | **15,968**<br><sub>(102 MiB / 47.9% CPU)</sub> | *Not possible* (no QUIC) | *Not possible* (no QUIC) | 🥇 **16,510**<br><sub>(158 MiB / 48.9% CPU)</sub> | **16,510**<br><sub>(158 MiB / 48.9% CPU)</sub> |
...
TWP÷YARP: H3→H1 @ `74ef56ee` ≈ **0.97×** RPS / **0.65×** Memory (102 / 158 MiB) — [32750437463](https://github.com/justcoding121/titanium-web-proxy/actions/runs/32750437463); other bridges @ `f9769503`: H3→H2 ≈ **1.08×** / **0.71×**; H2 TLS→H1 ≈ **1.14×** / **1.10×** Memory; H1→H2 ≈ **1.02×** / **1.12×** Memory. H1→H3 RPS ~tie (**1.00×**) with Memory win (**0.89×**). Prefer ratios over absolute RPS on GHA VMs. H1 plain / terminate / MITM rows are prior paste (no `compare-same` / `compare-mitm` this pass). nginx/YARP cannot MITM.

## Linux — Titanium vs nginx vs YARP

Median of **3 repeats** on `ubuntu-latest` (4 vCPU / 16 GiB). Saturation @ `f9769503` ([32737672381](https://github.com/justcoding121/titanium-web-proxy/actions/runs/32737672381)); bridges [32737668291](https://github.com/justcoding121/titanium-web-proxy/actions/runs/32737668291). **H3→H1** remasure @ `74ef56ee` — [32750437463](https://github.com/justcoding121/titanium-web-proxy/actions/runs/32750437463) (`compare-http3-cleartext`, same-job TWP+YARP+nginx). Warmup 2s / measure 8s; concurrency 8, 16, 32, 64. **Linux nginx is the authoritative nginx baseline.** The RPS workflow installs nginx.org mainline (`http_v3_module`) and `libmsquic` (`QuicListener.IsSupported=true` on `ubuntu-latest`). Prefer ratios over absolute RPS. **RPS cells** include peer `(MiB / CPU%)` as on Windows. Other bridge / H1 plain / terminate / MITM rows prior paste (no `compare-same` / `compare-mitm` this pass).

TWP÷YARP: H3→H1 @ `74ef56ee` ≈ **1.11×** RPS / **0.78×** Memory (141 / 180 MiB); other bridges @ `f9769503`: H3→H2 ≈ **1.06×** / **0.75×**; H2 TLS→H1 ≈ **1.10×** / **0.98×**; H1→H2 ≈ **1.03×** / **1.08×** Memory. H1 plain ÷nginx from prior `compare-same` ≈ **0.81**.
...
| Reverse | HTTP/3 · QUIC | HTTP/1 · plain | 🥇 **20,848**<br><sub>(141 MiB / 50.4% CPU)</sub> | **20,848**<br><sub>(141 MiB / 50.4% CPU)</sub> | **0**<br><sub>(101 MiB / 22.6% CPU)</sub> | **15,119**<br><sub>(101 MiB / 22.6% CPU)</sub> | **18,742**<br><sub>(180 MiB / 50.8% CPU)</sub> | **18,742**<br><sub>(180 MiB / 50.8% CPU)</sub> |
...
On this GHA shape, TWP H1 plain ÷ nginx H1 plain ≈ **0.83** (46,284 / 55,862). H1 TLS terminate ≈ **0.85** (36,518 / 43,011). TWP÷YARP H1 plain ≈ **1.12×** (46,284 / 41,203). H3→H1 @ `74ef56ee` ≈ **1.11×** RPS / **0.78×** Memory; other bridges @ `f9769503`: H3→H2 ≈ **1.06×** / **0.75×**; H2 TLS→H1 ≈ **1.10×** / **0.98×**; H1→H2 ≈ **1.03×** / **1.08×** Memory. Absolute RPS swings by VM; prefer the **ratio** and **median across repeats**. MITM publishes the same **15** Client×Origin pairs as Reverse (inspectable/decrypt), then dual-crypto extras (CONNECT, TLS↔TLS). nginx/YARP cannot MITM.

**nginx HTTP/3:** inbound QUIC terminate → cleartext H1 (`nginx-reverse-http3-cleartext`) @ `74ef56ee` `compare-http3-cleartext`: sustain **0** (p99/error SLO miss) / peak **15,119**. TWP/YARP H3→H1 on this row are from the same pass. nginx still cannot speak HTTP/3 to an origin (no H3 upstream in this conf).

**YARP HTTP/3 (this matrix):** TWP leads H3→H1 ≈ **1.11×** (20,848 / 18,742), H3→H2 ≈ **1.06×** (25,529 / 24,112). H1→H2 ≈ **1.03×** (28,343 / 27,456). H1→H3 ≈ **1.08×** (21,375 / 19,843). h2c→H3 ≈ **1.03×**.

**Windows vs Linux:** both CI envs are **4 vCPU / 16 GiB**, but do **not** compare absolute RPS across OS. Linux nginx leads H1 plain/TLS terminate (TWP second, ahead of YARP). Windows H3→H1 @ `74ef56ee` trails YARP slightly on RPS (**0.97×**) with a Memory win (**0.65×**). Cool laptop notes remain on [Performance Local Lab](Performance-Local-Lab).
