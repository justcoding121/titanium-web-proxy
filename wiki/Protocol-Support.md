...
| `Via` header injection | Yes (default) | `ViaHeaderPseudonym` defaults to `"titanium-web-proxy"`, appending `Via: {version} {pseudonym}` to all forwarded requests and responses (RFC 9110 §7.6.3). Set to an empty string to disable. Loop detection: an incoming request whose `Via` already contains the pseudonym is refused with `508 Loop Detected`. |
