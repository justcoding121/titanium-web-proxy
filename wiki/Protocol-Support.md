...
over HTTP/2 scaffolding, WebSocket frame validation, Via header injection, multipart streaming observer,
...
| Multipart/form-data boundary-aware streaming | Yes | Yes | N/A (not multipart-aware over h2 yet) | `OnRequestBodyWrite`/`OnResponseBodyWrite` receive boundary-delimited part data when the content-type is `multipart/form-data`. |
...
| `Via` header injection | Opt-in | Set `ViaHeaderPseudonym` to a non-empty token to append `Via: {version} {pseudonym}` to all forwarded requests and responses (RFC 9110 §7.6.3). Loop detection: an incoming request whose `Via` already contains the pseudonym is refused with `508 Loop Detected`. Disabled by default (empty pseudonym). |
...
| `ViaHeaderPseudonym` | `""` (disabled) | Token appended to `Via` headers on forwarded requests and responses. Loop detection rejects incoming requests whose `Via` already contains this token with `508 Loop Detected`. |
