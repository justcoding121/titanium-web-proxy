...
| Monitor.Enter_Slowpath ~9.5% after frame writer | syncblk + speedscope + pool-pick diag (`monitor-owners.md`, `pool-pick-shape.md`) | **~70%** Monitor = `TryPick`/`entry.Gate`; `ActiveStreamCount`→`ConcurrentDictionary.Count` (`AcquireAllLocks`) under Gate. **c=32:** 8 members, **0% soft-miss**. **c=64:** ~21% soft-miss + CreationGate at max (opens=0) | `REMEDIATION-PLAN.md`: (A) Interlocked active count; (B) skip CreationGate only on Gate-held `Count >= max`; (C) snapshot pick outside Gate if TryPick still &gt;~5% of Monitor after A+B |
