...
| H2→H1 MITM÷cleartext ≪0.90 after ssl-unwrap | Cool 3/5-rep High-perf (`h2h1-hostcache/`, `h2h1-gate12/`, `h2h1-final/`) + thread-time sample | Whole-RT SoftCap serialized warm SslStream reuse; `DataAvailable` closed keep-alives after exact-CL reads; per-request pool-key `StringBuilder`; cert callback `.Wait` on CompletedTask | **Create-only SoftCap(8)**; skip `DataAvailable` close on IsFastPath exact-CL; cache `CachedHttp11PoolKey`; sync cert validation. Cool 5-rep median **0.90** / mean **0.92** (`h2h1-final/`). Lock-free pool rent aborted (noisy dips) |
