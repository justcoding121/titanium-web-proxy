...
| H1→H3 “name normalize + tiny-body coalesce” looked like H1→H2 gap | Cool A/B (`win-parity-audit-20260822-*`) | Hypothesis: Title-Case QPACK tax + missing H1→H2-style fast commit / TLS coalesce | Implemented then **fully reverted**: cool H1→H3 fell ~**1.13× → 0.65×**. Baseline already at parity (~1.05–1.13×). Lesson: do not land probe-shaped encode/coalesce without a cool paired win — see [Performance](Performance) Windows notes |
| H2→H1 64 KiB ~0.87× YARP (tiny-GET already parity) | Cool pair + code compare vs Kestrel/YARP | Streamed path stripped Content-Length then empty END_STREAM DATA; pump wrote 8 KiB fills → 8 DATA frames + trailer; QueueDataFrame + flatten-coalesce memcpy | Keep CL + END_STREAM on last DATA; coalesce origin reads to 16 KiB frames. Cool ≈ **0.89×**. Framed CopyFrom + drop flatten-coalesce **regressed** ~0.65× — reverted. Next: find remaining copy/session tax without probe-shaped buffering |
